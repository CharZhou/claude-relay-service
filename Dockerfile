# 🎯 后端依赖阶段 (与前端构建并行)
FROM harbor.tegical.world/docker.io/node:18-alpine AS backend-deps
...
# 🎯 前端构建阶段 (与后端依赖并行)
FROM harbor.tegical.world/docker.io/node:18-alpine AS frontend-builder
...
# 🐳 主应用阶段
FROM harbor.tegical.world/docker.io/node:18-alpine
